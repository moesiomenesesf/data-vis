...
dateDim.group().reduceSum(d => d.facebook)
  ...
  main.variable(observer("googleByDayGroupPercent")).define("googleByDayGroupPercent", ["dateDim"], function(dateDim){return(
dateDim.group().reduceSum(d => d.googlePerc)
)});
  main.variable(observer("fbByDayGroupPercent")).define("fbByDayGroupPercent", ["dateDim"], function(dateDim){return(
dateDim.group().reduceSum(d => d.facebookPerc)
)});
  ...
  main.variable(observer("buildcomposite")).define("buildcomposite", ["md","container","dc","d3","dateDim","googleByDayGroup","fbByDayGroup"], function(md,container,dc,d3,dateDim,googleByDayGroup,fbByDayGroup)
{
  ...
  main.variable(observer("buildcomposite2")).define("buildcomposite2", ["md","container","dc","d3","dateDim","googleByDayGroupPercent","fbByDayGroupPercent"], function(md,container,dc,d3,dateDim,googleByDayGroupPercent,fbByDayGroupPercent)
{
...
crossfilter(dataset2)
  ...
  main.variable(observer("dateYearDim")).define("dateYearDim", ["facts2"], function(facts2){return(
...
facts2.dimension(d => d.Genre)
  ...
  main.variable(observer("moneyDim")).define("moneyDim", ["dateYearDim"], function(dateYearDim){return(
dateYearDim.group().reduceSum(d => d.Worldwide_Gross_M)
)});
  main.variable(observer("moneyGenreDim")).define("moneyGenreDim", ["genreDim"], function(genreDim){return(
genreDim.group().reduceSum(d => d.Worldwide_Gross_M)
)});
  main.variable(observer()).define(["moneyDim"], function(moneyDim){return(
moneyDim.all()
)});
  main.variable(observer("build3")).define("build3", ["md","container","dc","d3","dateYearDim","moneyDim"], function(md,container,dc,d3,dateYearDim,moneyDim)
{
  let view = md`${container('chart4','Total Arrecadado pelas Bilheterias por ano em Milhões de Dólares')}`
  let barChart = dc.barChart(view.querySelector("#chart4"))
  let xScale = d3.scaleTime()
                  .domain([dateYearDim.bottom(1)[0].date, dateYearDim.top(1)[0].date])
  barChart.width(800)
           .height(600)
